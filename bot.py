...
async def send_lesson_content(user_id, title, chat_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row:
        ...
        keyboard = [[
            ...
            InlineKeyboardButton("🗑️ حذف", callback_data=f"dl_{t_title[:10]}")
        ]]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def list_lessons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rows:
        ...
        for r in rows:
            ...
            keyboard.append([InlineKeyboardButton("🗑️ حذف", callback_data=f"dl_{row_id}")])
...
async def broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id == ADMIN_ID:
        ...
        # صيغة الرسالة الاحترافية المطلوبة
        announcement = (
            "⚠️ **إشعار تقني واعتذار رسمي**\n\n"
            "أعزائنا المشتركين، نود إعلامكم أنه قد واجهنا مؤخراً توقفاً مؤقتاً في خدمات البوت نتيجة خطأ برمجي بسيط (Bug) في أحد أسطر الكود المصدري.\n\n"
            "🛠️ **ما تم إنجازه:**\n"
            "• تم تحديد موضع الخطأ التقني ومعالجته بالكامل.\n"
            "• تم إعادة هيكلة النظام لضمان استقرار الخدمة وسرعة الاستجابة.\n\n"
